...
await page.goto(`http://localhost:${port}/?chart=${encodeURIComponent(songId)}&transpose=${transpose}&capo=${capo}`, { waitUntil: "networkidle0" });
await page.waitForFunction("document.querySelector('#chart.active') && window.ScoreCapture && window.ScoreCapture.info().lastBeat>0", { timeout: 10000 });
